...
export async function parseIntentToStory(intent: string, maxRetries = 3, forceMock = false) {
    if (forceMock) {
...
async function executeMockStory(intent: string) {
    const v1 = {
        type: 'line',
    ...
    const results = await Promise.all(visuals.map(async v => {
        try {
            ...
            return { ...v, data: await rowData.json() as any[] };
        } catch (e) {
