...
app.post('/api/query', async (req, res) => {
    ...
    try {
        ...
        const hydratedVisuals = await Promise.all(visualizations.map(async (v) => {
            try {
...
wss.on('connection', (ws, req) => {
    ...
    const interval = setInterval(async () => {
        ...
        try {
            ...
            const freshVisualizations = await Promise.all(currentQueryObj.visualizations.map(async (v) => {
                const data = await safeQuery(v.sql);
                return { ...v, data };
            }));
            ...
            console.error("WS Interval Error:", e.message);
        }
